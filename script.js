...
/* ============================================================
   9. CONTACT FORM — simple validation + fake submit
============================================================ */
(function initForm() {
  ...
  btn.addEventListener("click", () => {
    ...
    if (!name || !email || !phone) {
      ...
      [{ id: "name", val: name }, { id: "email", val: email }, { id: "phone", val: phone }]
        .forEach(({ id, val }) => {
  ...
  function shakeEl(el) {
    const moves = [8, -8, 6, -6, 4, -4, 0];
    moves.forEach((x, i) => {
      setTimeout(() => el.style.transform = `translateX(${x}px)`, i * 60);
    });
    setTimeout(() => el.style.transform = "", moves.length * 60 + 80);
  }
})();
